...
public class Item : MonoBehaviour
{
    ...
    public void DestroyItem(bool showScore = false, string anim_name = "", bool explEffect = false)
    {
		...
		//	StartCoroutine(WaitSeconds(_scaleTimeAnimation));
		//	Debug.Log ("end wait");
		//}
    ...
    {
		//if(_waving)
		//	yield return StartCoroutine(WaitSeconds(_scaleTimeAnimation));
	...
	public bool WaitForWave {get {return _waving;} set { _waving = value;}}
	bool _waving = false;
	float _scaleTimeAnimation = 0.5f;

	//public IEnumerator EnableWave()
	//{
	...
	//		if (items[i] != null)
	//		{
	//			Item _item = items [i].GetComponent<Item> ();
	//			StopCoroutine (_item.EnableWave ());
	//			_item.WaitForWave = false;
	//			//Debug.Log(center + "!!!!!!!!!!!");
	//		}
	//	}
	//	//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!//
